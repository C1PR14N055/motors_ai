...
from .adverts import AutoAdvert
import const
import network
import utils
...
def to_auto(data):
    ...
    advert.CountryID = guess_country(get_data_param('Tara de origine', data))
    ...
    advert.Price = utils.strip_number(data['list_label'])
    advert.PollutionNumber = guess_number('Emisii CO2', data)
    # options
    advert.CarOptions = guess_options(data)
    # ajax phone nr
    try:
        advert.Phone = network.get_av_phone(data)
    except:
        return advert, False

    utils.log(advert, const.LOG_LEVEL_LOW)
    return advert, True
...
def guess_category(categoryName):
    ...
    if utils.similar(tmpCategoryName, 'Cabrio') > 0.9:
        categoryName = 'Cabriolet/Roadster'
    if utils.similar(tmpCategoryName, 'Monovolum') > 0.9:
        categoryName = 'Microbuz/Furgoneta'
    if utils.similar(tmpCategoryName, 'Sedan') > 0.9:
        categoryName = 'Berlina'
    if utils.similar(tmpCategoryName, 'Coupe') > 0.9:
        categoryName = 'Coupe/Sport'
    if utils.similar(tmpCategoryName, 'Masina mica') > 0.9 \
            or utils.similar(tmpCategoryName, 'Masina de oras') > 0.9 \
            or utils.similar(tmpCategoryName, 'Combi') \
            or utils.similar(tmpCategoryName, 'Compacta') > 0.9:
        categoryName = 'Break'
    category, bid = guess('categories', categoryName, 1)
    if bid < 0.7:
        category = get_api_param('categories', 9)  # default to 'Altele'

    utils.log('%d%% sure %s = %s' %
...
def guess_brand_model(brandName, modelName, versionName):
    ...
    utils.log('%d%% sure %s %s %s = %s, %s' % (bmbid * 100, brandName,
                                               modelName, versionName or '-', brand['Title'], brandModel['Title']))
    return (brand['ID'], brandModel['ID'])
...
def guess_color(colorName):
    ...
    utils.log('%d%% sure %s = %s' % (bid * 100, colorName, color['Title']))
    return color['ID']
...
def guess_fuel(fuelName):
    ...
    utils.log('%d%% sure %s = %s' % (bid * 100, fuelName, fuel['Title']))
    return fuel['ID']
...
def guess_gearbox(gearboxName):
    ...
    utils.log('%d%% sure %s = %s' % (bid * 100, gearboxName, gear['Title']))
    return gear['ID']
...
def guess_country(countryName):
    ...
    utils.log('%d%% sure %s = %s' % (bid * 100, countryName, country['Title']))
    return country['ID']
...
def guess_status(statusName):
    ...
    utils.log('%d%% sure %s = %s' % (bid * 100, statusName, status['Title']))
    return status['ID']


def guess_boolean(booleanName, data):
    utils.log('100%% sure %s = %s' % (booleanName, True if get_data_param(
        booleanName, data) is not None and get_data_param(booleanName, data) else False))
    return True if get_data_param(booleanName, data) is not None and get_data_param(booleanName, data) else False


def guess_number(numberName, data):
    if get_data_param(numberName, data) is not None:
        return utils.strip_number(get_data_param(numberName, data))
    else:
        return None
...
def guess(param, searchTerm, parent=False):
    ...
    for p in params:
        if type(parent) == bool and parent:
            if 'Parent' in p and utils.similar(p['Title'], searchTerm) > bid:
                found = p
                bid = utils.similar(p['Title'], searchTerm)
        elif type(parent) == int:
            if 'Parent' in p and p['Parent'] == parent and utils.similar(p['Title'], searchTerm) > bid:
                found = p
                bid = utils.similar(p['Title'], searchTerm)
        else:
            if utils.similar(p['Title'], searchTerm) > bid:
                found = p
                bid = utils.similar(p['Title'], searchTerm)
